...
from clip_engine.job_control import JobCancelledError, check_cancelled
        ...
        try:
            with studio_job("export"):
                ...
                export_mode_label = str(
                    st.session_state.get("cs_export_mode_label", "Full frame fit with blurred background")
                )
                ...
                if not to_export:
                    ...
                    for idx, c in enumerate(to_export):
                        ...
                        pre_t0 = float(
                            st.session_state.get(f"start_{wid}", c.get("start_seconds", c.get("start", 0)))
                        )
                        pre_t1 = float(
                            st.session_state.get(f"end_{wid}", c.get("end_seconds", c.get("end", 0)))
                        )
                        pre_hook = str(
                        ...
                        write_clip_audit_json(
                            export_clip, session / f"clip_{idx+1:02d}_audit.json", index=idx + 1
                        )
                        ...
                        try:
                            result = export_vertical_clip_with_captions(
                                ...
                                dynamic_smart_crop=bool(
                                    st.session_state.get("cs_enable_dynamic_smart_crop", True)
                                ),
                            )
                            exported += 1
                    ...
                    if exported:
                        st.success(
                            f"Exported **{exported}** clip(s) to `{session.relative_to(PROJECT_ROOT)}`"
                        )
                    if failed:
            ...
            try:
                write_crash_report(e, context="export_batch")
            except Exception:
                pass
